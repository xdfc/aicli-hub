...
export class ClaudeDriver extends BaseCLIDriver {
  constructor() {
    super(
      'claude',
      'Claude Code',
      'claude',
      'Anthropic Claude Code CLI - AI coding assistant'
    )
  }

  async detect(): Promise<CLIInfo> {
    const available = await this.checkCommandExists('claude')
    let version: string | null = null

    if (available) {
      version = await this.getCommandVersion('claude', '--version')
    }
  ...
  async execute(prompt: string, options: ExecutionOptions = {}): Promise<ExecutionResult> {
    ...
    try {
      ...
      if (result.failed && result.exitCode !== 0) {
        error = result.stderr || 'Claude command failed'
      }
